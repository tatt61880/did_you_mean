...
function assertIsJugglable(SS, exp) {
	var actual = IsJugglable(StrToArr(SS));
...
function assertDidYouMean(SS, exp) {
	...
	var actual		= ListupDidYouMean(StrToArr(SS)).sort().ToStr();
	var expected	= exp.sort().ToStr();
...
function TestAll() {
	...
	if (failedCount == 0) {
		...
		var passedSpan = document.getElementById("PassedResult");
		passedSpan.appendChild( document.createTextNode("Passed " + passedCount + " tests.") );
		var failedSpan = document.getElementById("FailedResult");
